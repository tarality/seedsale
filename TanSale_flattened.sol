...
contract TanSale is ReentrancyGuard,Ownable,AccessControl{

    using SafeERC20 for IERC20;
    using SafeMath for uint;

    struct UserDepositInfo {
        uint256 amountDepositedUsdt; 
        uint256 amountDepositedUsdc; 
        uint256 amountDepositedEth; 
    ...
    event PresaleCreated( uint256 indexed _id, uint256 _totalTokens, uint256 _tokenPrice, uint256 _tokenPerUSD, uint256 _startTime, uint256 _endTime, bool buyWithEth,  bool buyWithUsdt, bool buyWithUsdc);  
    event PresaleUpdated( bytes32 indexed key, uint256 prevValue, uint256 newValue, uint256 timestamp );
    event Deposited(uint indexed _id, address indexed user, uint256 _amount, uint _paymentMethod, uint256 _timestamp, uint256 totalTokens);
    ...
    constructor(
        ...
        require(
        (_operator != address(0))&& (_treasury != address(0)) && (address(_usdt) != address(0)) && (address(_usdc) != address(0)) && ( _oracle != address(0)) && (_minDeposit != 0)
        );
          treasury = _treasury;
          USDT = _usdt;
          USDC = _usdc;
    ...
    function pausePresale(uint256 _id) external checkPresaleId(_id) onlyOperator {
        require(!paused[_id], "Already paused");
        paused[_id] = true;
        emit PresalePaused(_id, block.timestamp);
    }

    function unPausePresale(uint256 _id) external checkPresaleId(_id) onlyOperator {
        require(paused[_id], "Not paused");
        paused[_id] = false;
        emit PresaleUnpaused(_id, block.timestamp);
    }

    function createPresale(uint256 _startTime, uint256 _endTime, uint256 _tokenPriceInWei, uint256 _totalTokensToSell, bool _buyWithETH, bool _buyWithUSDT,bool _buyWithUSDC) external onlyOperator {
      require(
            _startTime > block.timestamp && _endTime > _startTime,
            "Invalid time"
        );
        require(_tokenPriceInWei > 0, "Zero price");
        require(_totalTokensToSell > 0, "Zero tokens to sell");
        presaleId++;

            uint256 valueWith6Decimals = _tokenPriceInWei.div(10e11) ;
        ...
        emit PresaleCreated(presaleId, _totalTokensToSell,_tokenPriceInWei,tokenPerUSD, _startTime, _endTime, _buyWithETH, _buyWithUSDT, _buyWithUSDC);
    }

    function Deposit( uint _id, uint _amount, uint256 _paymentMethod) external payable checkPresaleId(_id) nonReentrant returns (bool) {
     ...
     if (_paymentMethod == 0) {
        // Buy with ETH
        require(presale[_id].enableBuyWithEth, "ETH purchases not allowed");
        uint256 ethToUsd = calculateEthToUsd(msg.value);
        require(ethToUsd >= minDeposit, "Amount below minimum deposit");

        (, , totalTokens, ) = calculateBonus(_id, ethToUsd, false);
        require(leftToken >= totalTokens, "Insufficient tokens available");

        (bool success, ) = treasury.call{value: msg.value}("");
        require(success, "ETH transfer failed");

         userDepositInfo.amountDepositedEth = userDepositInfo.amountDepositedEth.add(msg.value);
     } else if (_paymentMethod == 1 || _paymentMethod == 2) {
        // Buy with USDT or USDC
        require(_amount >= minDeposit.div(1e12), "Amount below minimum deposit");

        address tokenAddress = _paymentMethod == 1 ? address(USDT) : address(USDC);
        bool isUsdt = _paymentMethod == 1;

        require(
            isUsdt ? presale[_id].enableBuyWithUsdt : presale[_id].enableBuyWithUsdc,
            isUsdt ? "USDT purchases not allowed" : "USDC purchases not allowed"
        );

        (, , totalTokens, ) = calculateBonus(_id, _amount, true);
        require(leftToken >= totalTokens, "Insufficient tokens available");

        IERC20(tokenAddress).safeTransferFrom(msg.sender, treasury, _amount);

        if (isUsdt) {
            userDepositInfo.amountDepositedUsdt = userDepositInfo.amountDepositedUsdt.add(_amount);
        } else {
            userDepositInfo.amountDepositedUsdc = userDepositInfo.amountDepositedUsdc.add(_amount);
        }
     } else {
        revert("Invalid payment method. Use 0 for ETH, 1 for USDT, or 2 for USDC");
      }
      ...
      emit Deposited(_id, msg.sender, _amount, _paymentMethod, block.timestamp, totalTokens);
    ...
    function calculateBonus(uint _id, uint256 amountDeposited, bool isUSDT) public view returns (uint256 extraAmount, uint256 bonusAmount, uint256 totalTokens,uint256 totalBonus  ) {
         bonusAmount = 0;
         extraAmount = 0;
         totalTokens = 0;
         totalBonus = 0;
    
         Presale storage sale = presale[_id];
        uint256 thresholdFactor = isUSDT ? 1 : 1e12;  // 1e12 for ETH, no factor for USDT
            ...
            for (uint256 i = 0; i < bonusThresholds.length; i++) {    
              ...
              if (amountDeposited >= threshold) {
               ...
               uint256 bonusToken = (sale.tokenPerUSD).mul(bonusThresholds[i].bonusPercent).div(1000);
               uint256 bonusPerUSD = (sale.tokenPerUSD).add(bonusToken);

               totalBonus = extraAmount.mul(bonusToken);
    ...
    function updateSaleTimes(uint256 _id, uint256 _startTime, uint256 _endTime) external checkPresaleId(_id) onlyOperator {
    ...
    function updateTanPrice(uint256 _id, uint256 _newPrice)  external checkPresaleId(_id) onlyOperator {
    ...
    function updatePaymentMethod(uint256 _id, uint256 _paymentMethod, bool _enable) external checkPresaleId(_id) onlyOperator {
        require(_paymentMethod >= 0 && _paymentMethod <= 2, "Invalid payment method");
        ...
        if (_paymentMethod == 0) {
            ...
            presale[_id].enableBuyWithUsdc = _enable;
        }

        emit PresaleUpdated( bytes32("PAYMENT_METHOD"), _paymentMethod, _enable ? 1 : 0, block.timestamp);
    }

    function updateThresholdBonus(uint256 _threshold, uint256 _bonusPercent, bool _isUpdate, uint256 _thresholdIndex) external onlyOperator {
    ...
    function updateMinDeposit(uint _amountInWei) external onlyOperator{
      require(_amountInWei > 0,"Amount Should be greater than Zero");
      minDeposit = _amountInWei;
    }

    function withdraw(address _token) external onlyOperator {
     if (_token == address(0)) {
